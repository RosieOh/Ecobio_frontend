...
<!--#Meta:modules/board/skins/coupon/css/write.css--><?php $__tmp=array('modules/board/skins/coupon/css/write.css','','','');Context::loadFile($__tmp);unset($__tmp); ?>
<!--#Meta:modules/board/skins/coupon/js/write.js--><?php $__tmp=array('modules/board/skins/coupon/js/write.js','','','');Context::loadFile($__tmp);unset($__tmp); ?>
<?php require_once('./classes/xml/XmlJsFilter.class.php');$__xmlFilter=new XmlJsFilter('modules/board/tpl/filter','insert.xml');$__xmlFilter->compile(); ?>
...
<?php if(!Mobile::isMobileCheckByAgent()){ ?><form action="./" method="post" onsubmit="return procFilter(this, window.insert)" class="boardWrite" ><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="act" value="<?php echo $__Context->act ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
    ...
    <?php if($__Context->module_info->content_use == 'y'){ ?>
        ...
        <input type="hidden" name="content" value="<?php echo $__Context->module_info->content_default ?>" />
        <?php }else{ ?>
...
$__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/coupon','_footer.html');
} ?>